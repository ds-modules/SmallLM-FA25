...
Basically the notebooks can also be understood as introducing four Python AI oriented packages: `GPT4all` is a package which facilitates the use of open source models in a Pythonic framework, `llama-cpp-python` provides more direct Python bindings to the llama.cpp inference engine, `langchain` is a framework for building applications with language models, and `openai` is a Python framework for interacting with OpenAI models via an API. The models can be stored and hosted locally.
...
## Notebooks 
 ...
 - [LangChain_SmallLM_Demo.ipynb](LangChain_SmallLM_Demo.ipynb) - **NEW!** An introduction to LangChain for building applications with language models. This notebook teaches the basics of prompt templates, chains, and memory using the same simple, freshman-friendly approach as other notebooks. Built on top of llama-cpp-python, it shows how LangChain simplifies common patterns like reusable prompts and stateful conversations. Perfect for students who want to build chatbots or more complex LLM applications.
